(() => {
  ...
  const BRAND_H = '★ 太子祭プリ ★';   // 平成の様式（★で挟む・16分割の見出しと同じ）
  const BRAND_R = 'taishisai puri';    // 令和の様式（小文字・字間広め）
  /* 平成のローマ字表記（2026-08-23 柄本 A-1）。
     🚨 ここが BRAND_R（令和の様式）になっていたため、平成のシールの下端に
     令和様式の小文字ローマ字が焼かれていた。焼き込み文字は客が何年も持ち帰るので、
     モードの線をまたいだまま出さない。平成の作法は「大文字・単語止まり・字間なし」
     （※柄本の様式判断で、一次資料の裏は取れていない＝直し方の中身は裁定領分） */
  const BRAND_H_ROMAN = 'TAISHISAI PURI';
  const BRAND_PLAIN = '太子祭プリ';    // 飾りなし（タイトル・扉・帯）。読み: たいしさいプリ
  const BRAND_SLUG = 'taishisai-puri'; // 保存ファイル名
  ...
  const MODES = {
    heisei: {
      ...
      /* 2026-09-04 更新（柄本の再検証 S-10・`data/work/heisei-jikki-sai-kensho-20260828.md`）:
         ...
         描画側に既にある fuchi / kira を平成の道具箱へ結線する（令和で使っているものと同じ実装）。
         ⚠️ 2026-09-09 訂正（Codexの検品）: 「どちらも strokePolyline に実装済み」は誤りだった。
            fuchi は strokePolyline の中の分岐だが、**kira は専用オブジェクト**として作られ
            （pointerdown で `{type:'kira', items:[]}`）、drawObject の 'kira' 分岐で描かれる。
            結線先が違うので、片方を直すときにもう片方も直る、とは思わないこと。
      ...
      sheet: {
        ...
        footerColor: '#a03cae',
        /* 🚨 ここは BRAND_R（令和の様式）にしないこと（2026-08-23 柄本 A-1 の再発防止）。
           平成のシールに令和様式の商品名表記が焼かれていた混入を直した。
           平成は BRAND_H_ROMAN（大文字・単語止まり）を使う */
        footerName: BRAND_H_ROMAN,
    ...
    { id: 'circleMix', label: 'まるMIX', gap: sp(20), radius: 0, shape: 'circle',
      /* 大きさ指定の丸型ミックス（大1+中1+小2）。ヘッダー下の描画領域に対する比率。
         🚨 2026-09-09: たて長(680×776)用に決めた比率をよこ長(900×556)へそのまま当てると、
            ...
            よこ長の紙面に合わせて置き直した。
            **定義から出る実際の直径は 大300 / 中240 / 小195・210px**
            （2026-09-09 Codexの検品で、ねらい値を書いていたコメントを実値へ訂正）。 */
      cellsNorm: [
  ...
  function warmSounds() {
    ...
    /* 最初の1組も遅らせる（この関数はタップの中からも呼ばれるため・2026-08-21）。
       🚨 この暖機は**サービスワーカーが操縦を持ったあと**に走るので、音声の取得は
       ワーカー越しになる。Chromeの音声要素はワーカー越しの応答に対していったん要求を
       切って範囲指定で取り直すため、`net::ERR_ABORTED` が音声の本数ぶん（約51件）
       ログに並ぶ。これは**故障ではない**。実測で確かめた副作用の無さ:
         ・オフラインで鳴らせるmp3 62/62（従来と同じ）
         ・3秒後には60本すべて readyState 4（すぐ最後まで鳴らせる）
         ・SWのキャッシュが76件そろうまで 1542ms → 1018ms（むしろ速い・3回とも）
         ・ページが回線から受け取る量 2132KB → 1477KB（ワーカーの棚から出るため）
       起動時に一斉preloadへ戻せばこの中断は1件に戻るが、
       そのぶん「反応できなかった時間」が 311ms → 439ms へ悪化する（実測）。 */
    idleStep(next);
  }
  ...
  /* ---------- 焼かれる範囲のガイド枠（2026-09-09 オーナー裁定・新設） ----------
     ...
     **四角の分割（4分割・2枚ワイド・6分割・16分割）では写真は1pxも切れなくなった**。
     ...
     🚨🚨 2026-09-09（Codexの検品指摘 P2）— **ガイドは drawShotFit と同じ計算で出すこと。**
     初版は円を写真の中心 (320,240)・半径240 に決め打ちしていた。平成（おさまりは 'center' 固定）
     では正しいが、**令和では嘘になる**:
       ・「顔に あわせる」… drawShotFit は shotFocus(i) で切り取り位置を顔へ寄せる。
         顔が右に寄っている写真では実際の円の中心は (400,240) まで動く。
         ガイドの中に描いたものが保存で消える／暗幕の下が実際には焼かれる、が両方起きる。
  ...
  function burnGuide(shotIdx) {
    ...
    const i = (shotIdx == null) ? curShot : shotIdx;
    const fit = photoFit();
    /* 丸セルの行き先は「1辺 2r の正方形」で、そこへ円のクリップがかかる（composeSheet と同じ）。
       写真座標に引き戻した円を返す。r の実寸は分割で変わるが、比だけ見ればよいので
       写真座標での半径だけを求める。 */
    if (isCircle && fit === 'contain') {
      // contain: 写真まるごとが正方形へ収まる → 写真座標での円は「中心・半径=長辺の半分」
      return { cx: SHOT_W / 2, cy: SHOT_H / 2, r: Math.max(SHOT_W, SHOT_H) / 2, firm: true };
    }
    // cover（center / face）: 見えるのは短辺の正方形。その正方形に内接する円が焼かれる
    const side = Math.min(SHOT_W, SHOT_H);
    let sx = (SHOT_W - side) / 2, sy = (SHOT_H - side) / 2;
    if (fit === 'face') {
      const f = shotFocus(i);
      if (f) {
        sx = Math.max(0, Math.min(SHOT_W - side, f.x * SHOT_W - side / 2));
        sy = Math.max(0, Math.min(SHOT_H - side, f.y * SHOT_H - side / 2));
      }
    }
    return { cx: sx + side / 2, cy: sy + side / 2, r: side / 2, firm: isCircle };
  }
  ...
  /* 🚨🚨 2026-08-28（オーナー実機指摘③「らくらく落書きの機能が、小さい。サイズが合ってなかった」）
     ...
     **56.9% × 65.2%（面積で 37%）** の箱に押し込まれた（実測: 364×313 / 640×480）。
     ...
     📌 2026-09-09 更新（Codexの検品で「旧寸法のまま」と指摘され、実測し直した）。
        台紙をよこ長(900×680)にし、セルを写真比に内接させたので、
        **四角の分割は全部 630×473 で同一**になった:
          4分割 630×473 ／ 2枚ワイド 630×473 ／ 6分割 630×473 ／ 16分割 630×473
          まる4・まるMIX 334×334（正円に内接する正方形。幾何学的にこれ以上広げられない）
        ＝ 四角どうしなら分割を選び直しても型の大きさが1pxも変わらない。
          これがオーナー実機指摘③「保存すると幅が修正されている」を閉じている本体。
        参考（履歴）: 8/23の共通安全域は分割によらず 364×313 の一択。
          8/28版（たて長台紙・分割ごと）は 4分割 408×473 ／ 2枚ワイド 630×350 ／
          6分割 630×473 ／ 16分割 409×473 だった。
  ...
  function buildPhotoFitRow() {
    ...
    PHOTO_FITS.forEach((f) => {
      ...
      b.addEventListener('click', () => {
        ...
        composeSheet();
        refreshPhotoPick();
        /* 🚨 2026-09-09（Codexの検品指摘 P2）: 焼かれる範囲のガイド枠は「おさまり」で形が変わる
           （contain なら写真座標での半径が 240→320 になる）。ここで描き直さないと、
           客はおさまりを変えた後も**古い枠**を見ながら描くことになる＝嘘の枠になる。 */
        renderDecoFramePreview();
      });
      row.appendChild(b);
    });
